...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        new_q_val = old_q + self.alpha * (new_val_estimate - old_q)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if epsilon:
            ...
            # weights = [self.epsilon/len(available_actions)] * len(available_actions)

            weights = [(1 - self.epsilon) if action == best_action else
                       (self.epsilon / (len(available_actions) - 1)) for action in available_actions]
